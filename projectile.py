...
def bunker_collision(projectile, bunker):
    left_edge = bunker.x
    right_edge = bunker.x + bunker.width   #these just give the boundaries to check whether a bullet collides with the bunker will implement in a different function
    bottom = bunker.y
    top = bunker.y + bunker.height
    if(projectile.x >= left_edge and projectile.x <= right_edge):
        if(projectile.y >= bottom and projectile.y <= top):         #implemented logic for bullet and return true if collision occured and false if not
            return True
...
# 18/04/26: Dillan van Wyk: Moved from class attributes to instance attributes
#19/04/2026: Denlan Molokwu: Implemented logic for bunker collision
class Projectile_Manager:
    ...
    def update(self, x_min, x_max, y_min, y_max, enemies,bunker, powerup_manager = None):
        if self.cooldown > 0:
            self.cooldown -= 1
